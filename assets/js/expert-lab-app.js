...
function getPerfumerIdentity() {
  try {
    const stored = localStorage.getItem('hallOfArtisans.perfumerId') || localStorage.getItem('hallOfArtisans.artisanId');
    if (!stored) return { hasPerfumerId: false };
    const data = JSON.parse(stored);
    const artisanId = data.artisanId || data.perfumerId || '';
    return {
      hasPerfumerId: Boolean(artisanId),
      creatorName: data.name || data.creatorName || 'Guest Perfumer',
      artisanId,
...
function updateStoryCardActions() {
  ...
  $('shareStoryCard').disabled = !identity.hasPerfumerId;
  $('downloadStoryCard').classList.remove('locked');
  $('shareStoryCard').classList.toggle('locked', !identity.hasPerfumerId);
...
$('shareStoryCard').addEventListener('click', () => {
  const identity = getPerfumerIdentity();
  $('storyCardMessage').textContent = identity.hasPerfumerId
    ? 'Download the card and upload it manually to Instagram Story.'
    : 'Create your Perfumer ID to download the 1080 × 1920 PNG story card.';
});

$('saveDraft').addEventListener('click', () => {
  ensurePerfumeName();
  $('formulaMessages').innerHTML = `<div class="message warn">"${state.perfumeName}" is ready to save to Hall Archive after login or Perfumer ID is enabled.</div>`;
});

$('refineFormula').addEventListener('click', () => {
  window.scrollTo({ top: document.querySelector('.formula-builder').offsetTop - 96, behavior: 'smooth' });
});
