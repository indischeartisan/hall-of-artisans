...
import type { ReviewRequest } from "../features/orders/types";
...
const recordTypeLabel = (type: RecordType) => type === "orders" ? "My Creations" : `My ${type[0].toUpperCase()}${type.slice(1)}`;

export default function MyArtisanIdPage() {
  useLegacyStylesheets("artisan-profile", artisanProfileStyles);
  const navigate = useNavigate();
  const { user, profile: authProfile, artisanId, loading: authLoading } = useAuth();
  ...
  useEffect(() => {
    ...
    if (!user) { setAuthFailed(true); setLoading(false); return; }
    if (authProfile?.display_name && artisanId) {
      setProfile({
        fullName: authProfile.display_name,
        artisanId: artisanId.public_id,
        specialty: String(user.user_metadata.specialty ?? "Artisan Perfumer"),
        status: artisanId.status === "active" ? "Registered Artisan" : artisanId.status,
        registeredAt: artisanId.issued_at
      });
      setAuthFailed(false);
      setLoading(false);
      return;
    }
    void authService.getArtisanIdentity().then(result => {
      if (!result.ok) { setAuthFailed(true); setLoading(false); return; }
      setProfile({ fullName: result.data.displayName, artisanId: result.data.publicId, specialty: result.data.specialty, status: result.data.status === "active" ? "Registered Artisan" : result.data.status, registeredAt: result.data.issuedAt });
      setLoading(false);
    });
  }, [artisanId, authLoading, authProfile?.display_name, user]);
  useEffect(() => { const refresh = () => { void orderService.getRequests(false).then(setRequests).catch(() => setRequests([])); }; refresh(); window.addEventListener("hoa:orders-change", refresh); return () => window.removeEventListener("hoa:orders-change", refresh); }, []);
