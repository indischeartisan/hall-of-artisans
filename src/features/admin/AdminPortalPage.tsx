...
import { staffService, type StaffAccess, type StaffRequestDetail, type StaffReviewer } from "./staffService";

const queueGroups = [
  { label: "New submissions", statuses: ["SUBMITTED"] },
  { label: "In artisan review", statuses: ["UNDER_REVIEW"] },
  { label: "Consultation", statuses: ["CONSULTATION"] },
...
function RequestWorkspace({ detail, access, reviewers, busy, error, onClaim, onAssign, onTransition, onMessage }: WorkspaceProps) {
  ...
  const canConsult = canReview && request.status === "CONSULTATION";
  const action = canReview && request.status === "SUBMITTED" ? ["UNDER_REVIEW", "Begin Artisan Review"] : canReview && request.status === "UNDER_REVIEW" ? ["CONSULTATION", "Open Customer Consultation"] : canReview && request.status === "CONSULTATION" ? ["READY_FOR_PAYMENT", "Ready to Begin Production"] : admin && request.status === "PAYMENT_PENDING" ? ["PAID", "Confirm Payment"] : admin && request.status === "PAID" ? ["IN_PRODUCTION", "Begin Production"] : admin && request.status === "IN_PRODUCTION" ? ["SHIPPED", "Mark as Shipped"] : admin && request.status === "SHIPPED" ? ["COMPLETED", "Confirm Delivery"] : null;
