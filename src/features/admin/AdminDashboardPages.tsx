...
export function AdminCreationsPage() {
  const { snapshot, reviewers, loading, error, refresh, loadReviewers } = useOutletContext<AdminOutletContext>();
  ...
  useEffect(() => { if (!reviewers) void loadReviewers(); }, [reviewers]);
...
export function AdminCustomersPage() {
  ...
  const customerAftercare = selected ? (aftercare ?? []).filter(item => item.customer.userId === selected.userId) : [];
  return <div className="hoa-admin-page"><PageHeader eyebrow="Relationships" title="Customers" copy="Open one customer record instead of searching across separate revision, reorder, and completed menus."/>
    ...
    <div className="hoa-customer-workspace"><section className="hoa-data-card"><table><thead><tr><th>Customer</th><th>Creations</th><th>Orders</th><th>Aftercare</th><th/></tr></thead><tbody>{filtered.map(customer => <tr key={customer.userId}><td><strong>{customer.name}</strong><small>{customer.artisanId}</small></td><td>{snapshot.creations.filter(item => item.customer.userId === customer.userId).length}</td><td>{snapshot.orders.filter(item => item.userId === customer.userId).length}</td><td>{aftercare ? `${aftercare.filter(item => item.customer.userId === customer.userId && item.status !== "RESOLVED").length} open` : "Loading…"}</td><td><button onClick={() => setSelectedUserId(customer.userId)}>Open</button></td></tr>)}</tbody></table>{!filtered.length && <p className="empty-copy">No customers match this search.</p>}</section>
      <section className="hoa-admin-card hoa-customer-summary">{selected ? <><header><div><small>Customer Record</small><h2>{selected.name}</h2><p>{selected.artisanId}</p></div></header><h3>Creations</h3>{creations.map(item => <a href={`/admin/creations?open=${item.request.id}`} key={item.request.id}><span><strong>{item.request.perfumeName}</strong><small>{item.request.requestNumber}</small></span>{badge(item.request.status)}</a>)}<h3>Orders</h3>{orders.map(item => <a href={`/admin/orders?open=${item.id}`} key={item.id}><span><strong>{item.orderNumber}</strong><small>{money(item.amount, item.currency)}</small></span>{badge(item.paymentStatus)}</a>)}<h3>Aftercare &amp; Follow-up</h3>{customerAftercare.map(item => <article key={item.id}><span><strong>{statusLabel(item.kind)}</strong><small>{item.subject}</small></span>{badge(item.status)}</article>)}{aftercare && !customerAftercare.length && <p className="empty-copy">No aftercare or repeat requests.</p>}</> : <p className="empty-copy">Select a customer to open their complete record.</p>}</section>
    </div>
  </div>;
}
