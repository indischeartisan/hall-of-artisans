...
import { materialCatalogAdminService, type CatalogMaterial, type CatalogMaterialSummary, type MaterialCategory, type MaterialInput } from "./materialCatalogAdminService";
...
export default function AdminLibraryPage() {
  ...
  const [materials, setMaterials] = useState<CatalogMaterialSummary[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [search, setSearch] = useState("");
  const [category, setCategory] = useState("all");
  const [status, setStatus] = useState("active");
  const [editing, setEditingState] = useState<CatalogMaterial | "new" | null>(null);
  ...
  const openEditor = async (id: string) => { setError(""); try { const material = await materialCatalogAdminService.get(id); if (!material) throw new Error("Material no longer exists."); setEditingState(material); } catch (cause) { setError(cause instanceof Error ? cause.message : "Material could not be opened."); } };
  const setEditing = (value: CatalogMaterialSummary | "new" | null) => { if (!value || value === "new") setEditingState(value); else void openEditor(value.id); };
