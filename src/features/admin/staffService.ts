...
export interface StaffAccess { signedIn: boolean; role: StaffRole | null; email: string }
...
const reviewFromRow = (row: ReviewRow): ReviewRequest => ({
  ...
  submittedAt: row.submitted_at, reviewedAt: row.reviewed_at, approvedAt: row.approved_at, paidAt: row.paid_at,
...
export const staffService = {
  async getAccess(): Promise<StaffAccess> {
    ...
    if (user.error || !user.data.user) return { signedIn: false, role: null, email: "" };
    ...
    return { signedIn: true, role, email: user.data.user.email ?? "" };
  },
