...
function ProjectWorkspace({ project, customerName, mode = "drawer", onClose, onChanged }: { project: ReviewRequest; customerName?: string; mode?: "drawer" | "inline"; onClose?: () => void; onChanged: () => Promise<void> }) {
  ...
  const content = <section className={`perfumer-project ${mode}`}>
    ...
    {detail?.messages.some(item=>item.attachmentUrl)&&<section className="perfumer-chat-attachments"><span>Customer images</span><div>{detail.messages.filter(item=>item.attachmentUrl).map(item=><a href={item.attachmentUrl} target="_blank" rel="noreferrer" key={`attachment-${item.id}`}><img loading="lazy" decoding="async" src={item.attachmentUrl} alt={`Chat attachment from ${staffSenderName(item.senderRole,item.senderName,customerName)}`}/><small>{formatDate(item.createdAt)}</small></a>)}</div></section>}
