...
import { subscribeToRequestUpdates } from "./requestLiveUpdates";
...
export function useOrderDetail(requestId: string | undefined) {
  ...
  const loadedRequestId = useRef<string | undefined>(undefined);
  ...
  useEffect(() => requestId ? subscribeToRequestUpdates(requestId, () => void refresh()) : undefined, [refresh, requestId]);
