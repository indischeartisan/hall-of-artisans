...
function OrderPickerItem({ item, currentId, includeDemo, onChoose }: { item: ReviewRequest; currentId: string; includeDemo: boolean; onChoose: (path: string) => void }) {
  return <button type="button" className={item.id === currentId ? "active" : ""} onClick={() => onChoose(`/my-orders/${item.id}${includeDemo ? "?dev=1" : ""}`)}><span><strong>{item.perfumeName}</strong><small>{item.requestNumber}</small></span><em>{WORKFLOW[item.status].label}</em></button>;
}
