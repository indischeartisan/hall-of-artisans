...
import { authPathWithReturnTo } from "../auth/returnTo";

const DRAFT_KEY="hallOfArtisans.describeCreationDraft.v1";
type LetterDraft=BespokeSubmissionInput;
const emptyDraft:LetterDraft={creationTitle:"",story:"",preferredNotes:[],notesToAvoid:[]};
const prepareAmbienceImage=(file:File)=>new Promise<{dataUrl:string;fileName:string}>((resolve,reject)=>{if(!["image/jpeg","image/png","image/webp"].includes(file.type)){reject(new Error("Please choose a JPG, PNG, or WebP image."));return}const reader=new FileReader();reader.onerror=()=>reject(new Error("The image could not be read."));reader.onload=()=>{const image=new Image();image.onerror=()=>reject(new Error("The image could not be previewed."));image.onload=()=>{const scale=Math.min(1,1200/Math.max(image.width,image.height));const canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(image.width*scale));canvas.height=Math.max(1,Math.round(image.height*scale));canvas.getContext("2d")?.drawImage(image,0,0,canvas.width,canvas.height);resolve({dataUrl:canvas.toDataURL("image/jpeg",.76),fileName:file.name})};image.src=String(reader.result)};reader.readAsDataURL(file)});
...
export default function DescribeCreationPage(){
  ...
  useEffect(()=>{
    ...
    setDraft({
      creationTitle:activeDraft.letter.creationTitle,
      story:activeDraft.letter.story,
      preferredNotes:[...activeDraft.letter.preferredNotes],
      notesToAvoid:[...activeDraft.letter.notesToAvoid],
      ambienceImage:activeDraft.letter.ambienceImage??null
    });
    setErrors([]);
    setMessage("");
  ...
  const chooseAmbience=async(event:ChangeEvent<HTMLInputElement>)=>{const file=event.target.files?.[0];event.target.value="";if(!file)return;setErrors([]);try{update("ambienceImage",await prepareAmbienceImage(file))}catch(cause){setErrors([cause instanceof Error?cause.message:"The image could not be added."])}};
  const save=async()=>{
  ...
  const preview=async()=>{const nextErrors=[];if(!draft.creationTitle.trim())nextErrors.push("Please give your creation a title.");if(!draft.story.trim())nextErrors.push("Please tell the artisan about the creation you imagine.");setErrors(nextErrors);if(nextErrors.length)return;try{const title=draft.creationTitle.trim();const describedDraft={creationTitle:title,story:draft.story,preferredNotes:draft.preferredNotes,notesToAvoid:draft.notesToAvoid,ambienceImage:draft.ambienceImage??null};const draftData={draftName:title||"Untitled Story Draft",perfumeName:title||undefined,letter:describedDraft,status:"ready" as const};const linkedDraft=activeDraft?await saveDraft(activeDraft.id,draftData):await createDescribedDraft(draftData);if(!linkedDraft)throw new Error("The current draft could not be linked to this preview.");const request=await orderService.createDescribedCreationPreview(describedDraft,linkedDraft.id);localStorage.removeItem(DRAFT_KEY);navigate(`/my-creations/${request.id}`)}catch(cause){const text=cause instanceof Error?cause.message:"The preview could not be created.";if(text.toLowerCase().includes("sign in")){localStorage.setItem(DRAFT_KEY,JSON.stringify(draft));navigate(authPathWithReturnTo("/artisan-login","/describe-your-creation?resume=review"));return}setErrors([text])}};
  return <><GlobalHeader activeLabel="Chamber of Creation" variant="light"/><main className="describe-shell"><section className="dc-hero"><div><span>⌁</span><h1>Describe Your Creation</h1><p>Describe the creation you imagine.<br/>Our artisans will review your story and transform it into a bespoke fragrance.</p></div></section><form className="dc-letter inner-panel bright-inner-panel" onSubmit={e=>{e.preventDefault();void preview()}}>
    ...
    <section className="dc-ambience"><header><span>03.</span><h2>Ambience Reference</h2><small>(Optional)</small></header><p>Add one image that captures the atmosphere, colors, place, or mood you want for the fragrance.</p><div className={draft.ambienceImage?"dc-ambience-box has-image":"dc-ambience-box"}>{draft.ambienceImage?<><img src={draft.ambienceImage.dataUrl} alt="Selected fragrance ambience reference"/><div><strong>{draft.ambienceImage.fileName}</strong><label>Replace Image<input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>void chooseAmbience(e)}/></label><button type="button" onClick={()=>update("ambienceImage",null)}>Remove</button></div></>:<label><span>＋</span><strong>Add Ambience Image</strong><small>JPG, PNG, or WebP · automatically optimized</small><input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>void chooseAmbience(e)}/></label>}</div></section>
    <section><header><span>04.</span><h2>Preferred Notes</h2><small>(Optional)</small></header><p>Notes, accords, or aromas you love</p><TagField value={draft.preferredNotes} onChange={value=>update("preferredNotes",value)} examples={["Tea","Rain","Vanilla","Fresh Air","Cedarwood","White Musk"]}/></section>
    <section><header><span>05.</span><h2>Notes to Avoid</h2><small>(Optional)</small></header><p>Notes or aromas you prefer to avoid</p><TagField value={draft.notesToAvoid} onChange={value=>update("notesToAvoid",value)} examples={["Smoke","Heavy Floral","Strong Oud","Too Sweet"]}/></section>
    {errors.length>0&&<div className="dc-errors" role="alert">{errors.map(error=><p key={error}>{error}</p>)}</div>}<div className="dc-actions"><button type="button" onClick={()=>setDraftsOpen(true)}>My Drafts</button><button type="button" disabled={saving} onClick={()=>{void save()}}>▣ &nbsp; {saving?"Saving...":"Save Draft"}</button><button type="submit">Review Your Creation &nbsp; ➤</button></div><p className="dc-message" role="status">{message}</p>
