...
export default function DescribeCreationPage(){
  ...
  const[draft,setDraft]=useState<LetterDraft>(()=>activeDraft?.letter??readDraft());const[message,setMessage]=useState("");const[errors,setErrors]=useState<string[]>([]);const[saving,setSaving]=useState(false);
  ...
  const preview=async()=>{const nextErrors=[];if(!draft.creationTitle.trim())nextErrors.push("Please give your creation a title.");if(!draft.story.trim())nextErrors.push("Please tell the artisan about the creation you imagine.");setErrors(nextErrors);if(nextErrors.length)return;const request=await orderService.createBespokePreview(draft);localStorage.removeItem(DRAFT_KEY);navigate(`/my-orders/${request.id}`)};
  return <><GlobalHeader activeLabel="Chamber of Creation" variant="light"/><main className="describe-shell"><section className="dc-hero"><div><span>⌁</span><h1>Describe Your Creation</h1><p>Describe the creation you imagine.<br/>Our artisans will review your story and transform it into a bespoke fragrance.</p></div></section><form className="dc-letter" onSubmit={e=>{e.preventDefault();void preview()}}>
    ...
    {errors.length>0&&<div className="dc-errors" role="alert">{errors.map(error=><p key={error}>{error}</p>)}</div>}<div className="dc-actions"><button type="button" disabled={saving} onClick={()=>{void save()}}>▣ &nbsp; {saving?"Saving...":"Save Draft"}</button><button type="submit">Preview Creation &nbsp; ➤</button></div><p className="dc-message" role="status">{message}</p>
  </form><section className="dc-journey"><header><h2>⌁ &nbsp; Your Bespoke Journey &nbsp; ⌁</h2><p>Here is the journey of your creation.</p></header><div>{steps.map(step=><article key={step.title}><i>{step.icon}</i><strong>{step.title}</strong><small>{step.text}</small></article>)}</div></section></main></>;
}
