...
export default function CustomerAftercarePanel({ requestId, demo = false }: { requestId: string; demo?: boolean }) {
  ...
  return <section className="fulfillment-panel aftercare-panel">
    ...
    {followUpId && <div className="aftercare-followup"><strong>New linked order created.</strong><span>Your completed original remains unchanged.</span><a href={`/my-orders/${followUpId}`}>Open New Order →</a></div>}
    {!followUpId && cases.filter(item => item.linkedReviewRequestId).map(item => <div className="aftercare-followup compact" key={`linked-${item.id}`}><strong>{label(item.kind)} order</strong><span>Linked to this completed commission.</span><a href={`/my-orders/${item.linkedReviewRequestId}`}>Open Order →</a></div>)}
