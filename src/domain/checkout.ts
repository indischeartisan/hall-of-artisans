...
export function validateCheckoutCandidates(items:readonly CheckoutCandidate[]):string|null{
  ...
  if(items.some(item=>item.status!=="READY_FOR_PAYMENT"||!item.finalPrice||item.finalPrice<=0||!/^[A-Z]{3}$/.test(item.currency)||!item.submissionId||!item.submissionSnapshot))return "Every creation must finish consultation, have an exact package price, and keep its submitted snapshot.";
