...
export function createArtisanBenchSubmissionSnapshot(draft: PerfumeDraft, materialNames: Record<string,string> = {}, createdAt = new Date().toISOString()): CreationSubmissionSnapshot {
  ...
  return clone({schemaVersion:SUBMISSION_SNAPSHOT_SCHEMA_VERSION,snapshotId:makeId(),sourceDraftId:draft.id,creationMode:"artisan_bench",title:draft.draftName,perfumeName:state.perfumeName||draft.perfumeName||draft.draftName,concentration:state.concentration,formulaMaterials:draft.formula.map(item=>({materialId:item.id,materialName:materialNames[item.id]??item.id,layer:item.layer??null,percentage:item.percent})),formulaMetadata:draft.formulaMetadata,fragranceBrief:draft.fragranceBrief??null,storyCardData:draft.storyCard??null,writtenStory:draft.fragranceBrief?.concept??"",preferredNotes:[],notesToAvoid:[],moodOrDirection:draft.storyCard?.directionTags??[],additionalNotes:state.perfumerNotes?.trim()??"",ambienceImage:null,createdAt});
}
