export type WorkflowStatus =
  | "DRAFT_PREVIEW" | "SUBMITTED" | "UNDER_REVIEW" | "CONSULTATION" | "READY_FOR_APPROVAL" | "REVISION_REQUESTED" | "READY_FOR_PAYMENT"
...
export const WORKFLOW: Record<WorkflowStatus, WorkflowStatusConfig> = {
  ...
  CONSULTATION:{key:"CONSULTATION",label:"Consultation",description:"Customer and artisan refine the creation together in chat.",allowedTransitions:[transition("READY_FOR_APPROVAL","reviewer","admin"),transition("CANCELLED","customer","admin")],customerVisible:true,chatAvailable:true,cancellationAvailable:true,checkoutAvailable:false},
  READY_FOR_APPROVAL:{key:"READY_FOR_APPROVAL",label:"Proposal Ready",description:"The artisan proposal is ready for the customer decision.",allowedTransitions:[transition("REVISION_REQUESTED","customer"),transition("READY_FOR_PAYMENT","customer"),transition("CANCELLED","customer","admin")],customerVisible:true,chatAvailable:true,cancellationAvailable:true,checkoutAvailable:false},
  REVISION_REQUESTED:{key:"REVISION_REQUESTED",label:"Adjustment Requested",description:"The artisan is reviewing the requested adjustment.",allowedTransitions:[transition("READY_FOR_APPROVAL","reviewer","admin"),transition("CANCELLED","customer","admin")],customerVisible:true,chatAvailable:true,cancellationAvailable:true,checkoutAvailable:false},
