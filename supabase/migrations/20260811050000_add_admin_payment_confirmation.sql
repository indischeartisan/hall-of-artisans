...
begin
  ...
  if next_stage='CONFIRM_PAYMENT' then
    if current_order.payment_status<>'pending' or current_order.production_status<>'not_started' then raise exception 'A pending, not-started order is required' using errcode='22023'; end if;
    update public.customer_orders set payment_status='paid' where id=target_order_id returning * into result;
    update public.review_requests r set status='PAID',paid_at=coalesce(paid_at,now()) from public.order_items i where i.order_id=target_order_id and r.id=i.review_request_id and r.status='PAYMENT_PENDING';
    event_name:='payment_confirmed'; event_label:='Payment confirmed by administrator';
  elsif next_stage='START_PRODUCTION' then
