create or replace function private.get_assigned_customer_summaries_internal(actor_id uuid)
...
as $$
begin
  if actor_id is null or actor_id is distinct from auth.uid() then
...
set search_path = ''
stable
as $$ select * from private.get_assigned_customer_summaries_internal((select auth.uid())); $$;

revoke all on function private.get_assigned_customer_summaries_internal(uuid) from public, anon;
grant usage on schema private to authenticated;
grant execute on function private.get_assigned_customer_summaries_internal(uuid) to authenticated;
